...
class WeeklyMealPlannerScreen extends StatelessWidget {
      ...
      child: Consumer<WeeklyMealPlannerViewModel>(
        builder: (context, viewModel, child) {
          return Scaffold(
            ...
            appBar: _buildHeader(context, viewModel, isDark),
            body: viewModel.isLoading
                ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFF50C878)),
                  )
  ...
  PreferredSizeWidget _buildHeader(
    ...
    WeeklyMealPlannerViewModel viewModel,
    bool isDark,
  ) {
    return AppBar(
...
class MealPlannerList extends StatelessWidget {
  ...
  Widget _buildDismissibleMealCard(
    BuildContext context,
    WeeklyMealPlannerViewModel viewModel,
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, i) =>
                        _buildSafeImage(mealPlan.selectedMeals[i].imageUrl),
                  ),
                ),
            ],
          ),
  ...
  void _showPlanDetailSheet(BuildContext context, MealPlanModel mealPlan) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final viewModel = Provider.of<WeeklyMealPlannerViewModel>(
      context,
      listen: false,
    );
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ...mealPlan.selectedMeals
                        .map(
                          (meal) => ListTile(
                            ...
                            subtitle: Text(
                              '${meal.kcal} kcal • ${meal.preparationTimeMinutes} min',
                    ...
                    _buildDetailRow(
                      Icons.access_time,
                      'Time',
                      mealPlan.specificTime.format(context),
                    ),
                    _buildDetailRow(
                      Icons.people,
                      'Servings',
                      '${mealPlan.servings} People',
                    ),
                    _buildDetailRow(
                      Icons.repeat,
                      'Repeat',
                      screen._getRepeatDaysText(mealPlan.repeatDays),
                    ),
                    const SizedBox(height: 16),
                    const Text(
                      'Notes',
                    ...
                    Text(
                      mealPlan.notes.isEmpty ? 'No notes' : mealPlan.notes,
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () =>
                        _confirmDelete(context, viewModel, mealPlan.id),
  ...
  Widget _buildDetailRow(IconData icon, String label, String value) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(icon, size: 18, color: const Color(0xFF50C878)),
